...
const loadFontsInline = require('../load-fonts-inline')
const { addPropertyMacro, overridePropertyMacro } = require('./helpers/macros')

test('loadFontsInline output', (t) => {
  const result = loadFontsInline()
...
test('loadFontsInline add option', addPropertyMacro, loadFontsInline)

test('loadFontsInline override option', overridePropertyMacro, loadFontsInline)
